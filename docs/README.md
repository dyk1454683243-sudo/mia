...
| [game-engine.md](game-engine.md) | `src/shared/mia.ts`: the hardcoded ranking, the phase machine, legal moves and auto-play, the doubt resolution, final standings, and the per-viewer redaction boundary. |
...
| [client.md](client.md) | The browser side: render-over-snapshot, the reconnect lifecycle and its stale-move stamp, the clock-drift correction, and why the countdown is updated by hand. |
